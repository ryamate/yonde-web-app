<ul class="nav nav-pills nav-justified my-2">
    <li class=" nav-item bg-white mx-1" style="border-radius: 12px">
        <a href="{{ route('users.index', ["name" =>  $user->name]) }}"
            class="nav-link px-1 pt-1 pb-0 btn btn-sm {{ $hasStored ? 'active text-white btn-outline-teal1' : 'btn-outline-teal1' }}"
    ...
    <li class="nav-item bg-white mx-1" style="border-radius: 12px">
        <a href="{{ route('users.likes', ["name" =>  $user->name]) }}"
            class="nav-link px-1 pt-1 pb-0 btn btn-sm {{ $hasLikes ? 'active text-white btn-outline-pink' : 'btn-outline-pink' }}"
            style="border-radius: 12px">
