...
<section>
    ...
    <div class="form-group">
        <div class="btn-group btn-group-toggle btn-block" data-toggle="buttons">
            <label
                class="btn btn-sm btn-outline-secondary {{ ((($storedPictureBook->read_status ?? old('read_status')) ?? 0 ) === 0) ? 'active' : '' }}">
                <input type="radio" name="read_status" id="read_status0" autocomplete="off" value="0"
                    {{ ((($storedPictureBook->read_status ?? old('read_status')) ?? 0 ) === 0) ? 'checked' : '' }}>
                きになる
            </label>
            <label
                class="btn btn-sm btn-outline-secondary {{ (($storedPictureBook->read_status ?? old('read_status')) === 1) ? 'active' : '' }}">
                <input type="radio" name="read_status" id="read_status1" autocomplete="off" value="1"
                    {{ (($storedPictureBook->read_status ?? old('read_status')) === 1) ? 'checked' : '' }}>
                よんだ
            </label>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <textarea type="text" name="review" id="review" rows="5"
            class="form-control">{{ $storedPictureBook->review ?? old('review') }}</textarea>
    </div>
